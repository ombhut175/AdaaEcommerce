...
const googleLogin = async (req, res) => {
    ...
    try {
        ...
        // Set the cookie with the actual response object
        setUserCookies(res, authToken);

        return res.json({ success: true, msg: "Login Successful", token: authToken, user });
    } catch (error) {
        return res.status(401).json({ success: false, msg: "Google authentication failed", details: error.message });
    }
}
